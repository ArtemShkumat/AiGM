...
The system fetches the relevant location JSON, merges data about local NPCs and quest hooks, and asks the LLM for a narrative.
The DM response describes the new scene and can optionally add new entities (e.g., “a mysterious peddler arrives”), which get saved in the JSON.
...
The LLM responds in character, referencing only what Elena knows or believes. The user can negotiate, ask for quests, or roleplay personal drama.
Quest Generation

If an NPC says, “I have a job for you,” a behind-the-scenes quest creation step might occur (with user approval).
The system prompts the LLM for a freeform quest, then a second prompt to convert it into quest JSON, storing multi-step outlines, fail conditions, and relevant new NPCs or locations.
